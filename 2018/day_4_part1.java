...
public class AdventOfCode2018_day4_1 {
    public static void main(String[] args)throws IOException {
        ...
        Object[] sleepnums = sleep.keySet().toArray();
        Object[] sleepvalues = sleep.values().toArray();
        
        int max = -1, save = -1;
        
        for(int i = 0; i < sleepvalues.length; i++) {
            int current = Integer.parseInt(sleepvalues[i] + "");
            if(current > max) {
                max = current;
                save = Integer.parseInt(sleepnums[i] + "");
            }
        }
        
        int max2 = -1, save2 = -1;
        
        ArrayList<Integer> ss = minutecount.get(save);
        for(int i = 0; i < ss.size(); i++) {
            int current = ss.get(i);
            if(current > max2) {
                max2 = current;
                save2 = i;
            }
